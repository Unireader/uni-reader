...
- **夜间模式切换慢（2026-07-26 用户反馈「切换夜间模式要等很久」）**：根因——切换后所有页键的
  夜间标志位翻转、缓存全 miss，实化窗口 + 预热页要在单串行队列上全部重渲 PDF 再反色。
  修法（`PageRenderEngine`）：反色（CIColorInvert+CIHueAdjust）是纯像素操作且自逆，新增
  `flippedNightKey`（`#n0`↔`#n1` 尾缀互换，基图/贴片同键规同走快路）——异色同参图在缓存
  就直接反转出图，只有缓存真没有才回退 PDF 重渲。整窗切换从重渲秒级变反转毫秒级。
