...
（空白 / 横线 / 方格 / 点阵 / 康奈尔 / 两栏），单页或批量设置。schema v17 新表 `board_page`，条目存页内坐标
（插页 / 删页 / 改尺寸不重写别的页的条目）；协议加 0x56~0x58 与 `boardAdd` 可选尾部。同日用户撤销了「安卓不改表结构」的规定、
定下「安卓模式1 与 Mac 功能对齐」的方向（`TODO.md` 第 8 条）。细节见 `BOARD-NOTE-PLAN.md §9`。

## ✅ 画板笔记：不挂 PDF 的独立无限白板，四端（2026-09-26，分支 `board-note`）

用户原话「做一个单纯的画板笔记，不需要 pdf，类似草稿纸模式，对应的功能可以迁移过去，pad 两个模式+网页需要适配」。
方案与实现记录全在 `BOARD-NOTE-PLAN.md`（§8 是文件地图、与方案的出入、已知遗留）。要点：schema v16 新表
`board_note` + `board_item`；Mac 上画板标签的会话 = 一张永远开着的草稿纸，草稿纸整条链路复用，落库改写到新表；
图片（拖入 / ⌘V / 插入，框选可移动缩放）；回收站、离线镜像、图片引用计数都覆盖到；协议加 0x52~0x55 + `GET /image`；
网页与安卓模式2 跟随书写 + 列出 / 打开 / 新建，图片只看不改；安卓模式1 独立读写并在缺表时补建（用户同意的例外）。
测试：`spike/board-store-test.swift` 37 项、编解码向量 +7（Swift / JS / 安卓三端一致）、镜像指纹向量 +2、安卓 JVM 单测 112 项全过。
...
动手前确认了四件事：回收站用**快照文件**（不是库内软删除）、覆盖**删文档 + 删笔迹图层**两种不可逆动作、
备份**只备 `library.sqlite`**、保留走**分级稀释**。

**动手前的现状**：侧栏右键「删除」点下去**没有任何确认框**，直接 `DELETE FROM document`，
`note` / `ink_layer` / `scratch_pad` 全部级联删掉；库平时也没有任何快照（只有建离线镜像时整库复制一次）。

- **回收站**：删之前把那些库行 `ATTACH` 进 `<工作区>/UniReader/Trash/<时间戳>_<标题>/snapshot.sqlite`，
  旁边一份 `manifest.json`（条数 / 引用到的图片 / 内容 hash）。**schema 仍是 v15，安卓端一个字不用改**
  ——删除对三端来说和以前完全一样（硬删、照常传播）。🔴 归档成功才允许删，顺序反了就等于没做。
  - 恢复分两种：那篇书还不在库里 → 整份放回去，**document id 原样**（`unireader://` 链接和 MCP 里
    记的 id 仍有效）；那份 PDF 已经被重新导入过 → **并入**现有那篇（`variant.content_hash` 有 UNIQUE
    约束，照搬必撞），面板上会写清楚「将把 N 笔笔迹并入《…》」。这是最常见的现实场景：误删 →
    重新拖进来 → 才发现笔记没了。
  - 图片本体的 30 天清理**给回收站让路**：文档一删它引用的图立刻变成孤儿并开始计时，保留期设成
    90 天 / 永不时会先一步被清掉、恢复只剩空框。护身符 = 各 manifest 里的 `images`。
  - 侧栏删除现在有确认框，写明「2616 笔笔迹、12 条笔记、30 处高亮」以及能从哪里放回来。
  - `ocr_page` / `page_geom` / `page_align` **刻意不进快照**：它们按 content_hash 存、删文档根本不会
    删它们，留在主库里同一份 PDF 重新导入立刻复用。PDF 本体同理（删文档本来就不删 `PDFs/` 里的副本）。
- **定时备份**：`checkpointTruncate()` + `VACUUM INTO` 出 `UniReader/Backups/library-<时间戳>.sqlite`
  （临时名 → rename，半截文件不算数）。打开工作区后 8 秒 / 每 6 小时 / 退出前各一次，距上次不足
  30 分钟就跳过。保留 = 最近 5 份 + 每天 1 份留一周 + 每周 1 份留一个月，约 16 份封顶；
  认不出命名的文件一个都不碰。
  - 还原 = 先把当前库另存成一个**永不被稀释**的还原点 → 关连接 → 换文件（`-wal`/`-shm` 一并删）→
    **退出 App**。不原地重开：库连接、标签页、笔迹按页窗口、笔架、草稿纸、Agent 面板全挂在当前这个
    store 实例上，原地换库等于要求每一处都正确丢弃并重建状态，而这个功能一年用不上一次。
- 入口：「文件 › 回收站…」「文件 › 工作区备份…」，设置 ›「通用」多一个「备份」小节（开关 / 频率 /
  保留期 / 按工作区列一行）。中英文案都补齐了。
- **落地时抓到的两个真 bug**（都是写测试才露出来的）：① 归档失败后的清理会把**已经存在的**快照文件
  删掉——「回收站里有一处会删已有快照」这件事本身就不成立，改成只清自己刚建的那一份；
  ② `DateFormatter` 会吃掉多余部分去认一个前缀，`library-.sqlite` 这种空串它也给得出日期，
  于是文件名长度自己卡死 15 个字符。
- 测试：`spike/trash-test.swift`（62 项，归档 → 删 → 恢复的库回环，含并入分支和默认图层那条老行判定）、
  `spike/backup-retention-test.swift`（39 项，分级稀释 + 文件命名）。
- **用户实测当场报的一条**：设置页里「保留期」「备份频率」两个下拉**选不了**。原因是它们绑的是
  `Binding(get:set:)` 包住的静态属性，body 里没有任何 SwiftUI 状态被读到 → 选完视图不重算 →
  控件按旧值画回去。改成 `@AppStorage` 绑同一批 UserDefaults 键，副作用（重建定时器）挪进 `.onChange`。
  这条已记进 `AGENTS.md` 的「关键坑」。

## ✅ 跳转到指定页 ⌃G（2026-09-21）

- 「视图」菜单加「跳转到指定页…」，默认 **⌃G**（用户指定；⌘G 是系统「查找下一个」的口径，不占）。
  快捷键可改——它走的是 `ShortcutAction.gotoPage`，设置 ›「快捷键」›「视图」里就能换。
- 按下弹一张系统提示框：输入框预填当前页码并全选，直接打数字就覆盖，回车 = 「跳转」。
  页码按人用的 1 起算，超范围夹到两端，输入不是数字就当没按。
- 跳转走 `session.jump(kind: .list)`——与点目录 / 点缩略图同一条路，所以进得了跳转历史、⌘[ 回得来。
- 空标签 / Markdown 笔记标签没有「第几页」可言：菜单项灰掉（`MainMenu.validateMenuItem`），
  快捷键随之失效。⌃G 连按不叠第二张表单。

## ✅ 阅读进度偶发跑到别的页（2026-09-21，查清 + 修）

用户报「本来在 88 页，切到别的 PDF 再切回来变成 112 页」。这类偶发、事后才发现的问题没法靠复现调试，
于是先加了一支专用日志通道，当天就当场抓到现场。

- **日志通道 `ProgressLog`**（`Sources/App/ProgressLog.swift`，`[PROG]` 前缀，**保留**）：开关口径同
  `PadLog`——`touch ~/Library/Logs/UniReader-progress.log` 开、`rm` 关；关着时热路径上只剩一次
  「每秒最多一遍」的 `fileExists`，写盘在独立队列。打点覆盖**所有**会改 `document.read_*` 的路径：
  存（带**为什么存**：翻页 / 滚动 / 缩放 / 切走 / 关闭标签 / 对齐切换 / 尾随补存 + 用的是哪条锚点）、
  两条守卫拦下的「跳过」、真正落库与读库、开文档恢复、切回标签、非本机锚点
  （restore / pad / toc / search / link / mcp）、本机滚动的**跳变**（带当时几何）、
  阅读区首帧落位 / 宽度重排 / 换基准、页高布局的来源、以及**离线镜像合并改写进度**
  （`document` 是整行 upsert，安卓那侧读到哪会合并回来——这条路径其余打点全看不见）。
- **真因**：`ReaderView.refit()`（窗口可用宽变了，如开合侧栏 / Inspector、拖窗口）在一个 CATransaction 里
  「先按新基准重排、再把画面钉回原处」，而改 `fitBasis` / `docView.frame` / `magnification` 每一步都会
  **同步**触发 clip view 的 bounds 通知 → `maybeEmit()` 拿**新基准**去解读**还没校正的旧滚动偏移**，
  算出一个离谱的页码，当成「用户滚动到这里」上报并存库。日志原文（同一秒，注意顺序）：
  ```
  滚动跳变 p88+0.457 → p112+0.650  topDocY=159001.9 ds=1.085 fit=1085.0 clipY=172447.2 docH=352128.5
  锚点 mac(跳变) p112+0.650 seq=3
  重排(宽度变了) 钉住 p88+0.457  fit 1385.0→1085.0 zoom=1.097 缩放过=是
  ```
  画面随后被正确钉回 p88，但**库里那条错的没人纠正**（refit 后 0.3s 内不再上报，位置也没再变），
  切走再回来读库就落在 p112。各处 `suppressEmitUntil` 都是**重排做完之后**才设的，挡不住这中间的自发上报。
- **修法**：加一道 `ReaderView.relayouting` 门（半成品状态不许上报位置），包住 `refit` / `rebase`（⌘0）/
  `applyZoom`（⌘滚轮，走这条路时 `isZooming` 恰好是 false，挡不住）/ `setupIfPossible` 四处的
  「重排 + 钉回」事务；`refit`、`rebase` 结束时顺带把 `lastEmitted` 同步成钉住的那个位置，
  免得下一次去重 / 跳变判断错位。**只挡上报，不动画面**，零闪烁纪律不受影响。

## ✅ Markdown 笔记正文搜索（2026-09-21）

- 阅读窗工具栏原有的搜索框现在同时支持 PDF 与 Markdown 笔记；切到 Markdown 标签后，⌘F 直接搜索当前笔记正文。
- 搜索以 Markdown 引擎编辑器里的**实际显示文本**为准（不是拿可能长度不同的 storage text 硬套选区），大小写与音调符号不敏感；状态胶囊沿用 PDF 的「第 N 项 / 共 M 项」「无匹配项」，上下按钮循环跳转。
- 当前命中用系统文本选区与查找指示器显示，并自动滚到可见位置；正文继续以文件为唯一真源，搜索只读，不改写任何文字或 `[[…]]`。
- 正文编辑期间若搜索仍开着，会在引擎同步完显示文本的下一拍重新统计命中。PDF 原有搜索链路不变。Debug 编译通过。
- 活动标签是 Markdown 笔记时，阅读窗口标题显示笔记名；切回 PDF 后仍显示书名与页码。
- 内置 Agent 每次发送消息都会重新识别活动标签：Markdown 标签会带上笔记来源、相对路径与链接；MCP
  get_state 报出标签类型，get_current_view 返回当前编辑器的实时正文（包括尚未自动保存的改动）。Markdown
  正文可由 update_markdown 显式修改：先读 revision、再提交完整新正文；期间用户有新编辑就拒绝覆盖，写成功后
  文件与所有正在显示该笔记的编辑器同步更新。
- 标签组现在按类型保存 PDF 与 Markdown 身份；重启后恢复混合标签的原顺序和活动项。旧版 PDF-only
  记录可继续读取；恢复后台 Markdown 标签时只恢复身份，不提前创建编辑器。
...
**Mac 端用户同日实测通过**：「效果很好，文字全部不斜了！ocr 效果也会好很多，更匹配了」（OCR 识别的是转正后的页图，行框与文字贴得更准）。

- **先量再做**（用户给的《软件工程 2024张琼声》228 页）：页内歪斜中位 0.38°、最大 1.35°（截图里「左边缘越往下越靠左」就是它）；
  相邻两页正文栏中心相差中位 13pt、最大 27pt，奇偶页各偏一边；页宽 489.6~506.9pt → 按页宽铺满时字号有 3.5% 的跳动。
  能测到边的页栏宽全是 396pt ±1 → 扫描比例一致，只要「旋转 + 平移」。
- **用户拍板**：第一版方案是「存原始坐标、显示时换算」（要改约 35 处换算）；用户回「数据可以清理掉，我还是希望功能优先」→
  改成**对齐后的页面直接当页面坐标**，笔迹 / 高亮 / 图钉 / 框选代码一行不动，改动收敛到出图口（`PageBitmap` 的 `align` 参数刻意无默认值）+
  原生页坐标互转（`PageGeometry`）+ 缓存键（`displayKey`）。同批做安卓模式1；开关放「视图」菜单；切换时不换算批注、弹窗提示条数、清 OCR。
- **测量算法**（`App/ScanAlign.swift`）：投影轮廓锐度找歪斜角（**线性分摊到相邻两格**——整格计数在 0° 有取整假峰，原型里 21 页被误判成 0°；
  切行要合并 ≤1.5pt 的碎缝，否则像素行与格子错位把一行切碎、整页测不出行）；转正后按「≥25% 的行共用」找栏左右边缘；
  栏宽互校 + 前后 8 页同奇偶页兜底。这本书 183 页两边都测到、23 页一边、22 页用邻页值。叠 40 页对比图：对齐后连汉字格子都重合。
- **Debug 包慢 22 倍**（228 页 -O 2.2s / -Onone 49s）：数组下标与 `append` 在 -Onone 里是真函数调用。`@_optimize(speed)` 单加只到 46s；
  内层循环改裸指针 + `while` 后 **Debug 6.3s / 优化 1.9s**，结果逐位不变（参数表戳前后都是 `604cec38`）。
- 平板（web 采集页 / 安卓模式2）**零改动**：`layout.v`/`docId`、`toc`/`bookmarks` 的 `docId` 改发 `displayKey`（未开对齐与从前逐字节相同），
  页尺寸与 `/page.png` 都按对齐后的出。离线镜像 `page_align` 另开通道：按 `updated_at` 整行取新，写入硬盘方向要人工确认（它决定页面坐标）。
- **安卓模式1**（`android/` 仓库，细节见 `android/AGENTS.md` 与方案 §7）：`shared/ScanAlign.kt` 移植 + `PdfSource` 报对齐后页尺寸、
  出图走「中间图 + `Canvas.drawBitmap(Matrix)`」（pdfiumandroid 2.0.1 的带矩阵出图把两个斜切项传反，不能用）；schema 升 v14；
  内容 hash 改取**真正打开的那个文件**所属版本（此前划字取「第一个版本」，单版本的书两者相同）；镜像通道同 Mac。
  戳的跨端向量 `7438e8a2` 两端（`shasum` 与 Swift）一致。`assembleDebug` 通过、JVM 单测 111/0（新增 `ScanAlignTest` 11 + `MirrorDiffTest` 3）；
  插桩用例 `MirrorApplyTest` 只编译未跑。**平板上的实际观感（转向 / 清晰度 / 最宽档内存峰值）待用户真机看。**
- 验证：`scan-align-test` 44/44、`mirror-align-test` 22/22、`scan-align-real`（真 PDF 出叠加图）；回归 `store-test` 54、`ocr-store-test` 15、
  `image-store-test` 39、`mirror-build` 50、`mirror-diff` 58、`mirror-autopush` 17、`mirror-multi` 27、`image-mirror` 31、`mirror-fp` 50、
  `page-layout` 25、`page-disk-cache` 11、`canvas-margin` 24 全绿（`mirror-apply` 49/50，失败的那条是 TODO 里记着的日期相关老问题）；`xcodebuild` Debug 通过。
