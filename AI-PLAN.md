...
> **2026-09-12：站点已撤掉这条控件**（用户实测只剩一种模式，图片上传照常）。内置表不再填 `modes`，
> 模式菜单随之隐藏、投递前不再找控件；下面的机制**整套保留**，站点哪天再长出模式，外部配置
> `ai-providers.json` 填上 `modes` + `mode_for_image` / `mode_for_text` 就回来，不必重编译。
> 待验清单第 5 条随之作废。
