...
>
> **状态：已完成并合回 `main`（2026-09-02，合并点 `fd2c5ff`，回退点 tag `pre-appkit-window`）。**
> M0~M5 全部走完，§7 的验证清单十条已由用户真机逐条过完。
> 本文**留着不删**：§5.1 那张坑表是这次迁移唯一的产出沉淀，同类问题下次直接查表。
...
5. 关窗：进度落库、移动硬盘能立刻弹出（引用全放掉）——**这条最要紧**，它是 2026-08-05 那笔
   「必须退出整个 app 才能弹盘」的账，而本轮又动过标签装载与进度保存两处；用户已实测正常；
