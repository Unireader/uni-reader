...
-1. **模式2 多工作区切换 + 撤销/重做新图标真机验**（2026-09-06 落地，实现记录见 `HISTORY.md` 同日条目）。
   - 验：① Mac 上开两个及以上工作区时，平板标签页栏**只列当前工作区那几篇**（此前是全部窗口混排）；
     ② 工作区芯片 `⌄` 点开列出 Mac 上开着的工作区（名字 + N 篇 + 当前打勾），点一行切过去，
     标签页/书库/目录整套跟着换；③ 切回原工作区落在**上次待的那一篇**而不是第一篇；
     ④ Mac 那边关掉某个工作区的最后一个窗口后，平板的芯片列表里那一项也没了；
     ⑤ 顶栏尺子右边那两个键现在是不是一眼能认出「撤销/重做」。
   - 已知留白：**工作区改名后平板上仍显示旧名**——`workspaceName` 是窗口快照，只在
     `workspace.$documents` 变化时重 sync，改名不触发（`library` 的 wsName 一直有同一个毛病）。
     分组不受影响（同一工作区的窗口共用一份 `WorkspaceManager`，旧名也一致），重开窗口即恢复。
...
0.5 **编辑撤销/重做 + 笔迹剪贴板**（2026-09-02 落地，实现记录见 `HISTORY.md` 同日条目）。
   ...
   **剩下这几条还没单独过**（顺手碰到就试，不必专门排）：
   ...
   - **平板三端（同日落地，待真机验）**：web 采集页 / 安卓模式2 / 安卓模式1 都加了撤销·重做 +
     剪切·复制·粘贴（顶栏；剪贴板三件只在框选模式下出现）。要验的：
     ① 平板上撤销/重做后 **Mac 与平板两边是不是都跟着回退**（模式2 走的是 Mac 那一份栈）；
     ② 平板复制 → **Mac 上 ⌘V** 粘出来（同一份系统剪贴板），反向也试；
     ③ 平板粘贴落点是不是**视口正中**（平板没有指针，与 Mac 的「指针处」刻意不同）；
     ④ 草稿纸开着时平板按撤销，撤的是纸上那一步；
     ⑤ **模式1（离线）**：撤销/重做走本机栈（**只管笔迹，注解不进栈**——与 Mac 端有差距，
     有需要再补）；剪贴板是进程级的，跨标签页/跨文档粘贴要试。
   - **安卓按钮整体收 15%**（`Ui.TOUCH` 48→41、`ICON` 22→19、`TopBar.BAR_H` 56→48）：
     全 App 的图标按钮一起变小了，**要看的是「会不会点不准」**（顶栏密排、草稿纸浮条、抽屉里的
     小按钮）。嫌大/嫌小都只改 `Ui.kt` 那两个常量。
...
2. **笔&笔架&笔迹四项真机回归**（2026-07-26 已落地，见 HISTORY）：尺子吸附手感 / 局部擦除两端一致 / 本机落墨（含 ⇧ 尺子）/ 框选移动（含重开文档位置保持、平板镜像）——手测发现问题即在此开新条目。
   - **尺子线的粗细**（2026-09-06 修，见 HISTORY 同日）：从前整条线的宽度取自「抬笔前最后一个采样」的
     压感（≈0）→ 细成头发丝；现在锁成**这一笔的峰值压感**、两端同值。要验的是这个粗细合不合手感
     （是不是「按多重画多粗」），以及平板本地即时回显与 Mac/落库那份粗细一致。三端都要看
     （网页采集页 / 安卓模式1 / 安卓模式2 + 各自的草稿纸）。
...
- **安卓划字（2026-09-04 新做）待真机验证 + 两处已知缺口**：
  - **待验**：选字模式的手指划选（起选/拖到另一行/双击选整行/单击收起）、四色高亮与「批注」
    落库后在 Mac 上打开是否位置一致、图钉是否真的跟 macOS 一个样（固定大小、选区注解落行末右侧）。
    全部只跑过 JVM 单测与编译，**一次真机都没上过**（手上没有设备）。
  - **缺口①：笔记类型的图标只映射了 7/16 个 SF Symbol**（`local/NoteTypeIcons.kt`）。
