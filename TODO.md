    ...
    **三端都做但实现分期（Mac 先行 → 网页 → 安卓两模式）**。
    - **Mac 待真机验**：① 同一页加两枚不同位置的书签 → 目录里同页两行，按页内位置先后排；
      ...
      **目录项自身顺序一个没动**；④ 没有目录的书：书签平铺树顶、点得动；⑤ 页号在第一个一级组
      ...
      ⑪ **页右缘的书签缎带**（同日加；首版橘色圆图钉用户「根本没注意到」，已改成红缎带）：
      位置对不对（纵向应当落在书签自己的页内位置上，同页多枚不重叠）、会不会挡正文、
      点开的小面板（名字+页码+改名/删除）好不好按、缩放/画板模式下跟不跟得住页边、
      夜间模式下看得见吗、与另两种圆图钉一眼分得开吗。
    - **合并规则的参照实现 = `Sources/App/TOCMerge.swift`**（纯函数），覆盖测试
      `spike/toc-merge-test.swift`（19 项全绿）。web/安卓照它实现，别各自照文字再推一遍。
    - **还没做**：协议那两条（`bookmarks` 0x4D / `bookmarkEdit` 0x4E）—— 做 web 或模式2 时再
      搬进 `PROTOCOL.md` 并补跨端向量；web 端与安卓两模式的 UI。
    - 🔴 做安卓之前先核**模式1 读同一个库**时是否按 kind 分流（它现在就会读到不认识的 `kind=5`）。
