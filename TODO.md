...
- 已完成清单（工作区持久化/多窗口/采集页/方案 B/阅读区 v2/T1-T3 OCR/笔迹持久化/笔架/二进制协议/UDP/各批 bug 修复）**见 `HISTORY.md`**。当前要点：
  ...
  - **2026-09-13：图片笔记落地（Mac + 离线镜像），待用户真机验证**（方案 `IMAGE-NOTE-PLAN.md`，规格 `REQUIREMENTS.md §1.10`，
    ...
    ✅ 用户 2026-09-13 实测功能正常。**UI 二轮**（同日，用户四条：窄边 / 缩放默认关设置里开 / 图片气泡去铅笔改右键 /
    文字 12pt）已改，记录见 `HISTORY.md` 同日条目；**待验**：固定尺寸口径下两种气泡的观感、设置开关切换后即时生效、
    图片气泡右键菜单与点图看原图、竖图气泡收窄（✅ 用户同日实测通过）。
    **Markdown 编辑器已接**（同日，`swift-markdown-engine` 0.9.0，项目第一个 SPM 依赖，见 `HISTORY.md`）；
    ✅ 编辑器用户实测可以。**气泡也换成引擎只读渲染**（同日二轮，阅读区「纯 SwiftUI」红线的唯一例外，`AGENTS.md` 已记）；
    **待验**：气泡展开时有没有一帧跳高（第一帧按估计占位、引擎排完再对齐）；鼠标停在气泡上滚轮能不能滚阅读区；
...
3. **三端笔迹绘制对比工具 —— 已落地**（2026-09-07，实现记录见 `HISTORY.md` 同日条目）。
   `spike/ink-cross/run.sh` 一条命令跑三端出图 + 报告。**改任何一端的笔迹渲染之后顺手跑一次**
   ——它就是用来照出「改一边忘了另两边」的。首轮修掉两条真分叉（钢笔锥度、铅笔三道纹理），
   剩下的两处小差异见下面「已知欠账」。

4. **MCP 服务（给外部 Agent 用）——三批已落地合入 `main`；批 1 实测通过、批 2/3 待实测**（2026-09-13，**`MCP-PLAN.md`**）。
   ...
   **现状**：批 1 七个工具 + 设置 › Agent 页 + 口令/监听地址全在（方案 §15，用户已实测通过）；批 2 五个工具
   ...
   `spike/mcp-server-test.swift` 31 项）全绿。已合入 `main`（2026-09-13，合并提交 `7a79546`）；批 2/3 与
   §17.3 那条图钉修复一起等用户实测；批 1b（stdio 桥接）/ 1c（新版协议）视需要。
