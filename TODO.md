...
- **`broadcastStrokes` 是 O(n²)：每收笔一次就把整篇文档的笔迹重发一遍**（2026-08-28 查闪烁时发现，
  用户观察到的「e2e 一直增长」就是它）。`strokes` 是全量镜像，每条 `ink end`（以及擦除时的**每一批**
  8ms 擦除点）都触发一次 `AppModel.broadcastStrokes`，payload = 全文档可见图层的所有点，
  `pt3` 12 字节 → 几百条笔迹就是几百 KB ~ 几 MB 一发，写得越久越慢。三处成本叠加：主线程上把每个点
  装箱成 `[[String: Any]]`＋`NSNumber`、`WireCodec` 再走一遍、然后整包上 WiFi。
  **闪烁已与它解耦**（见 HISTORY 2026-08-28），但延迟与带宽还在。
  修法要拍板：① 加增量 opcode（纯追加走 `strokesAppend`，擦除/框选/切图层仍回落全量）——真正解决，
  但要动 `PROTOCOL.md` + 三端；② 只砍常数（`[InkStroke]` 直编字节跳过 Foundation 装箱 + 擦除期广播合帧
  到一帧一发）——不动协议，估计能省 2~3 成，治标。
  现场数据看安卓 logcat 的 `回推 strokes N条/M点 ≈XKB ackRel=… e2e=…`（1s 一条）。
