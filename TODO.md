...
9. **框选三增强真机验证**（2026-08-17 三端落地，见上）：① 自由框选：拖出不规则闭合区域，凹形区域凹槽内的笔迹应**不**被选中、边缘擦到的笔应算选中；跨页拖时路径点应 clamp 在起笔页内。② 光晕：选中的每笔都有蓝色包边，哪些被框住一目了然；移动/缩放 ghost 期间光晕应跟着预览走。③ 缩放：拖角手柄默认等比（Mac 上 ⇧ 放开两轴）、拖四边中点手柄单轴拉伸，笔迹大小/线宽随框变化，注解跟着走位（字号不变）；缩小到极值不消失（线宽下限 0.5）、贴页边 clamp 不越界。④ 回归：框内拖动移动、单击/Esc/切工具清选中、隐藏图层笔迹不被命中。**三端各验一遍**：Mac 本机（已初步验过手感）；web 平板（模式2：提交后 Mac 复判结果与本地预览一致、乐观渲染不闪回）；安卓模式1（本地落库：缩放后笔宽变化持久化、重开文档保持）与模式2（同 web）；安卓验过后再提交安卓仓库。
