...
> 🔴 **2026-09-07：本节里所有「待真机验证 / 待用户真机验证 / 待真机确认」字样一律作废**
> ——用户统一裁决为通过（「测试部分可以认为都正常了」），归档见 `HISTORY.md`
> 「真机验收批量结清」。**没有逐条改写这几十处措辞**，读到时按已通过理解即可。
> 真正还没做的事只在下面「⏭️ 接下来」和「🐞 已知欠账」两节里，别从本节找待办。
...
> **2026-09-07 大扫除**：此前这一节积压了 19 条「待真机验证」，用户统一裁决为通过，
> 整批迁往 `HISTORY.md`「真机验收批量结清」。本节从此**只留真的没做的事**，
> 验收类条目不再挂在这里。
...
2. **三端笔迹绘制对比工具**（2026-09-07 用户定，代替已放弃的 Rust 统一核心）。

   路线图 ⑤（Rust 笔迹核心，`INK-CORE-UNIFY-PLAN.md`）2026-07-30 已搁置、2026-09-07 正式放弃。
   但它当初要解决的问题还在：**三端笔迹渲染是各自独立实现，观感已经分叉且没人说得清差在哪**。
   用户拍板改走「先量出来，再各端各修」——**做一个对比工具，不做统一实现**。

   - **要找的差异**（`INK-CORE-UNIFY-PLAN.md §1` 列过，目前一条没修）：
     马克笔叠笔接缝变深 / pad 实时反馈阶段铅笔无抖动纹理 / fountain 起收锥度 /
     ballpoint 半透明接缝黑点。
   - **形态**：一份**共同的笔迹输入向量**（固定点集 + 压感 + 笔型 + 笔宽，走
     `wire-cross-test`/`mirror-fp-vectors` 那套「只许在末尾追加」的纪律）→ 三端各自渲染成 PNG
     → 汇总成一张逐笔型并排的对照页（外加差值图，肉眼比对靠不住）。
   - **各端出图的现成手法**（都有先例，不用从零想）：
     · **Mac**：`ImageRenderer` spike，同 `spike/scratch-look.swift`；
     · **web**：headless canvas 截图，playwright 现成；
     · **安卓**：模拟器插桩测试渲染到 `Bitmap` 落 app 专属目录再 `adb pull`,
       同 `androidTest/.../shared/RadialIconProbeTest.kt`（那次就是靠它逐像素抓出
       density 单位混用的 bug）。
   - 🔴 **这是对比工具，不是统一实现**：找到差异后**各端各修、谁碍眼修谁**（2026-07-30 定的口径，
     没有变）。别借着做工具的机会又去抽公共渲染层。
   - 🔴 **跨端抄绘制几何时先确认坐标单位**：web canvas 是 px、安卓 `dp(常量)` 是「dp 常量折 px」，
     混用就是 density 倍数的错——圆盘图标那次就栽在这儿。

3. **真平板方案 B 画质/带宽打磨**：缓冲本地滚动 + progressive 多清晰度。
...
- **三端笔迹观感分叉**（马克笔叠笔接缝变深、pad 实时反馈阶段铅笔无抖动纹理，及
  `INK-CORE-UNIFY-PLAN.md §1` 列的其余几条）。原打算靠路线图 ⑤ 三端算法统一一并解决，
  该路线已放弃 → **归到上面「接下来」第 2 条**：先用对比工具量出来，再各端各修。
...
- **路线图 ⑤ Rust 笔迹核心：放弃**（2026-07-30 搁置 → 2026-09-07 用户正式放弃）。
  ...
  随之作废。它要解决的观感分叉改由「接下来」第 2 条的对比工具接手。
