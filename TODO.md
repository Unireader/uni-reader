...
- 已完成清单（工作区持久化/多窗口/采集页/方案 B/阅读区 v2/T1-T3 OCR/笔迹持久化/笔架/二进制协议/UDP/各批 bug 修复）**见 `HISTORY.md`**。当前要点：
  ...
  - **2026-09-03：扫描件的平铺水印块不再进文字选择（Mac，待真机验）**。用户报：扫描件上大块的
    独立水印（「王道计算机教育」，常被裁掉半截、字也认错）被 OCR 认成一堆大字块散在正文里，
    拖选正文会带出一串水印碎片。新增 `Sources/App/OCRWatermark.swift`（纯函数，测试
    `spike/ocr-watermark-test.swift` 29 项 + 真实库回归 `spike/ocr-watermark-real.swift`）：
    - **判据全是几何 + 跨页统计，不认具体文字**（换出版社同样有效）：候选 = 行高 ≥ 2.5×页内行高
      中位数 **且包围盒近方形**（w/h ∈ [0.7, 3.5]——n 个字水平排 w/h≈n、竖排 ≈1/n，只有斜排 45°
      才接近正方，于是竖排框图标签与整行大字标题压根进不了候选）；判定 = 位置格（4% 网格 + 3×3
      邻域）或归一化文本在 ≥ max(8, 6%×页数) 页重复。样本不足时另有**同页伙伴**兜底
      （一页里 ≥3 块大方块、彼此文本有 ≥2 字公共子串）。
    - **指纹一次性从库里读全书**（`LibraryStore.allOCRPayloads`，解码放后台）——阅读区逐页懒加载
      攒不出跨页样本，而库里往往整本都跑完了；边识别边读的新书每多攒 8 页重建一次。
    - 落地面：`DocSession.ocrVisibleRuns(page:)` 是**唯一出口**（选择/复制/⌘A/OCR 搜索/分组/调试
      上色全走它），`ocrRuns` 仍是真源（落库与建指纹用）；`ocrGroups` 改按可见行算，下标一一对应。
      OCR 面板加开关「忽略平铺水印块」（默认开），调试上色里被剔除的块画成**灰色虚线框**。
    - 实测（用户那本《王道 2027 计算机组成原理》340 页 21842 行）：候选 2569、判水印 2563，
      剩下 6 条全是正文章标题（「第2章」「第4章」…），**零误伤**；同库另一本无水印的排版书候选数 0
      （不动一行）。只喂 6 页样本时靠同页伙伴仍抓到 42/48。
    - **待真机验**：① 拖选正文是否还会带出水印；② 章标题/框图标签能不能照常选；③ OCR 搜索结果
      是否干净；④ 开关关掉后水印块是否立刻回来。
