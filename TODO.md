...
- 已完成清单（工作区持久化/多窗口/采集页/方案 B/阅读区 v2/T1-T3 OCR/笔迹持久化/笔架/二进制协议/UDP/各批 bug 修复）**见 `HISTORY.md`**。当前要点：
  ...
  - **2026-09-10：笔迹内存——点压到 f32 + 按页窗口装载/淘汰，已落地，待用户实测**（方案与落地记录
    ...
    **用户定的验证方式（2026-09-10）：不做一次性全量测试，边用边测，有问题再报。** 首批日志（新包，同日 15:19）：
    ...
    切标签 48~87ms。**日志里现在最大的项与笔迹无关**，先记着、不动：
    - 「王道计组」冷开 788ms：装载 366ms 里 `目录 143`（`TOCEntry.build` 遍历 PDF 大纲）、`注解 154`
      （`loadTextNotes`：整行 `[String: Any]` 读法 + 每条 `JSONDecoder`，且可能含 Foundation 首次解码的预热；
      账本没记条数，下次先把条数记进 detail 再判）、`OCR 28`、`草稿纸 29`（kind=4 仍是同步整篇读+解码，量小）。
    - 「打开」路径首帧实化的是 **p1–1**（`首批页图 +450(实化 p1–1 需渲1)`），随后才跳到进度页 p285——
      p1 那张渲染白做（`首张页图 +669(p1 缓存)`，约 200ms 主线程），切标签路径没这问题（快照种子直接是 p284–286）。
      锚点恢复（`emitAnchor(origin: "restore")`）晚于首帧实化，值得单独看一次。
