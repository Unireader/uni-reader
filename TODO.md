...
- **打包**：非沙盒 + 公证发布流程。
- **保存 PDF 上次打开的缩放**（TODO，2026-07-21 记）：目前重开只恢复阅读进度（页 + 页内比例 `read_page/read_frac`），**缩放不恢复**（每次回 fit-width）。需把 `zoom`/`fitBasis` 语义持久化到 document（新增列或存 meta），重开时复原用户上次的放大倍率。注意与 `PageStreamView` 的 fit 基准/窗口 refit 逻辑对齐（存的是「相对 fit 的 zoom」还是「绝对页宽」要想清楚，跨窗口宽度不同）。
