...
- **网页采集页文字笔记**（2026-07-26 用户提出）：平板网页端可添加文字笔记（不需要先选中文字的那种，自由放置），通过圆形选择器（呼应环形选笔盘的交互形态）显示/编辑。
- **macOS 端网页服务地址显示优化**（2026-07-26 用户反馈）：`ServerPanel` 的采集页地址支持一键复制（点击复制按钮）；换行显示有点毛病（长地址换行截断不美观），一并优化。
- **快捷键体系**（2026-07-26 用户提出）：
  - **Cmd+F 触发搜索**（搜索改 `.searchable` 后快捷键没接上，必须补）；
  - 整体补一批快捷键：笔型切换（钢笔/马克笔/橡皮等）、模式状态切换（如临时笔迹模式、夜间模式）等，先列清单再定键位。
