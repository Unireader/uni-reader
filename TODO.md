...
- 已完成清单（工作区持久化/多窗口/采集页/方案 B/阅读区 v2/T1-T3 OCR/笔迹持久化/笔架/二进制协议/UDP/各批 bug 修复）**见 `HISTORY.md`**。当前要点：
  ...
  - **2026-08-17：Mac 端框选三增强**（自由框选 + 选中笔迹光晕 + 角手柄缩放；web/安卓端待后补）：① 矩形框选改为**自由路径框选**（`lassoPath` 视口坐标 ≥3pt 抽稀 → 页内归一化不规则多边形，`InkEdit.pointInPolygon` 射线法命中，边界算内；跨页点 clamp 到锚点页边缘的「仅页内」语义不变）；② 选中笔迹画 accent 色**光晕边缘**（`lassoStrokeHalo`，内容坐标 Canvas，线宽=笔宽×zoom+5pt、透明度 0.35，所见即所选；注解仍由高亮框覆盖）；③ 高亮框加**缩放手柄**（`LassoHandle`：四角 = **等比**缩放（⇧ 临时自由两轴）、四边中点 = **单轴**缩放，anchor = 对侧手柄，10pt 命中，accent 色半透明圆点样式），拖动中按瞬态 `lassoGhostScale` 预览（clamp 0.05...20），松手 `InkEdit.scaled` 一次性提交（点集绕 anchor 按轴缩放 + clamp 0...1、线宽 ×√(sx·sy) clamp 0.5...40、注解 anchor/rects 同缩放、padSession 镜像广播——与 move 同一套落库/对账机制）。手势起点判定顺序：手柄→缩放，框内→移动，其余→新框选。`spike/ink-edit-test.swift` 扩到 62 项全绿 + `xcodebuild` 通过，**待用户验证**（见「接下来」第 9 条）。
...
9. **Mac 框选三增强验证**（2026-08-17 落地，见上）：① 自由框选：拖出不规则闭合区域，凹形区域凹槽内的笔迹应**不**被选中、边缘擦到的笔应算选中；跨页拖时路径点应 clamp 在起笔页内。② 光晕：选中的每笔都有 accent 色包边，哪些被框住一目了然；移动/缩放 ghost 期间光晕应跟着预览走。③ 缩放：拖角手柄默认等比（⇧ 放开两轴）、拖四边中点手柄单轴拉伸，笔迹大小/线宽随框变化，注解跟着走位（字号不变）；缩小到极值不消失（线宽下限 0.5）、贴页边 clamp 不越界；padSession 窗口操作后平板镜像到位。④ 回归：框内拖动移动、单击/Esc/切工具清选中、隐藏图层笔迹不被命中。
