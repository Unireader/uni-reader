...
- 构建 / 结构 / 红线 / 文档地图见 `AGENTS.md`。三端（Mac / web 采集页 / 安卓两模式）的跨端契约 = `PROTOCOL.md`
  + 工作区 SQLite schema（现 v15，`Sources/Store/`）。
...
6. **Agent 面板后续批次**（`ACP-AGENT-PLAN.md §5`，A1 已于 2026-09-18 落地）：
   ...
   （A3 块级 Markdown 2026-09-20 已做：回复与思考正文改用笔记那套引擎只读渲染，`ACP-AGENT-PLAN.md §7`；
   A4 独立窗口吸附、两块内置面板改为与阅读区并排，2026-09-18 已做。）

7. **Markdown 笔记：还差的一件**（`MARKDOWN-NOTES-PLAN.md §7`，三批都在 2026-09-20 落地）。

   能用的：**两种源**——内建 `Notes/`（「导入笔记目录…」= 整个目录复制进来）与**引用的外部目录**
   （「引用笔记目录…」= 不复制、就地编辑）；侧栏每个源一段、按**真实目录层级多级展开**；
   标签页里编辑（自动保存）；⌘⇧N 新建；右键改名 / 在访达里显示 / 移到废纸篓 / 取消引用；
   `unireader://…&md=`；`[[…]]` 单击跳转。

   🔴 **笔记正文一个字都不改**（用户 2026-09-20 定）：`[[…]]` 按名字解析，**改名 / 挪目录就断链**，
   这是明确接受的代价，别再想着「帮用户把链接修好」。

   - **输入 `[[` 的自动补全**（候选列表面板；引擎已经把「光标在 `[[` 里」这个状态给出来了）。
   🔴 **整条链路还没拿真 vault 跑过**：纯函数与文件层在 `spike/markdown-link-test.swift` 里绿了（66 项，
   含「复制之后正文逐字相同」「原目录没被动过」两条），界面这一层是编译通过、**没有实测**。
   第一件事：拿一个真 Obsidian 目录，**导入**一次 + **引用**一次，看四件事——多级目录在侧栏对不对、
   `[[…]]` 点得动吗、`![[attachments/图.png]]` 显示得出来吗、原目录有没有被动过（一个字节都不该动）。

8. **安卓模式1 与 macOS 版功能对齐**（2026-09-26 用户定：「模式1 作为独立功能需要达到所有能和 macOS 版本一样的功能，
   部分无法实现的则跳过」）。同日用户撤销了「安卓不改表结构」的规定（schema 仍是两端共同的契约，见 `android/AGENTS.md`）。
   - 第一步：列一份 Mac ↔ 模式1 的功能差距清单（已知差距：Markdown 笔记、图片笔记的新建与编辑、模式1 撤销栈不含注解、
     回收站与定时备份、书签以外的若干检查器功能等——以清单为准），逐条标「要做 / 平板做不到跳过（写明原因）」。
   - 之后 Mac 每加一个功能，默认模式1 同步跟上。
   - 已知差距（2026-09-26 分页画板时记下）：模式1 的画板没有撤销栈（Mac 有）；模式1 分页画板没有「在页上长按弹菜单」，
     页操作都在工具条「页面」弹层里、作用于视口中心那一页；模式1 画板上的图片只能看不能加 / 挪 / 删。
...
- **笔记小窗（`NoteWindowController`）拖不到别的显示器**（2026-09-24 用户报，根因已查清，
  **用户定先记录不修**）：`show(attachedTo:)` 用 `host.addChildWindow` 把小窗做成阅读窗的真 AppKit
  ...
  `RefWindowController`（参考窗）用的是同一套 `addChildWindow` 机制，大概率同病，只是还没人反馈。
  要支持跨屏得放弃子窗口关系、改成纯 `NSWindow.level` 悬浮（同层级效果但不是真的父子窗口），
  代价是「跟着阅读窗一起移动 / 一起最小化」不再是系统免费给的，得自己监听阅读窗的移动/最小化通知
  手动同步；「关阅读窗时一起关」这条相对好办（观察 `NSWindow.willCloseNotification` 即可）。
  动手前再跟用户确认一遍要哪种取舍：① 全面换成悬浮 level（彻底跨屏，不跟随移动/最小化）；
  ② 同屏时维持现在的子窗口行为，一旦被拖到别的屏再自动摘掉子窗口关系换悬浮 level（观感最接近现状，
  实现更复杂，摘除/重新挂回的时机要想清楚）。
...
- **Markdown 笔记的几处留白**（`MARKDOWN-NOTES-PLAN.md §7`，2026-09-20 刻意不做）：
  `[[名字#小节]]` 的锚点只解析到文档级；改名 / 挪目录会断链（不改别人正文的直接代价）；
  离线镜像不合并 md 正文、引用源根本不同步。
