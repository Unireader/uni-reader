...
14. **笔迹镜像改增量真机验证**（2026-08-28 落地，见 `HISTORY.md`；Mac 与安卓都要重装）：
    ① **e2e 不该再爬**：写满几百笔之后 e2e 读数应当基本平着走，而不是随笔迹数往上抬。
       `adb logcat -s UniReader/Canvas | grep 回推`——写字期间应当基本都是
       `回推 追加 1条/…点 ≈几KB`，且那个 KB 数**不随时间涨**；只有擦除/框选/切图层/切档才见「全量」。
    ② **一笔都不能丢**：连续写一整页，中途翻页/切档/刷新平板（重连会补全量），笔迹应当分毫不差；
       Mac 上和平板上对着看一遍。
    ③ **不能重影**：收笔那一下屏幕上不该出现「同一笔叠了两条」再消失（乐观笔销账与追加是同一次操作）。
    ④ **擦除回归**：连续擦一片，不能出现「删掉了又出现」；擦完再写，新笔迹照样上屏。
    ⑤ **框选移动/缩放**：提交后镜像回来位置对、不闪。
    ⑥ **图层显隐**：切可见性后平板上的笔迹跟着增减（走的是全量那条路）。
    ⑦ **网页采集页**同样跑一遍 ①②③（它没有乐观笔迹，只验丢没丢、位置对不对）。
...
- ~~`broadcastStrokes` 是 O(n²)~~ **2026-08-28 已修（待真机确认）**：加了增量 opcode
  ...
  仍发全量。详见 `HISTORY.md`。**剩下的两条尾巴**（都不阻塞，等真机数据再定要不要动）：
  - **擦除路径仍是全量**（每收一批擦除点广播一次，已被发送端合帧压到 ~1 帧/RTT）。要做成增量得给
    笔迹上**稳定 id**，而线上是刻意不带 id 的（见 `PROTOCOL.md` 的 `lassoMove`），改动量大得多。
  - **全量帧的编码仍走 Foundation 装箱**（每个点一个 `[NSNumber]`）。改成 `[InkStroke]` 直编字节
    能省掉主线程上那几十万次分配；线上字节数不变，所以只在擦除/框选/切档那几下有意义。
