...
- **擦除闪**：根因在 **Mac 端** ——`AppModel.inkErase` 是**每收到一批擦除点就 `broadcastStrokes()` 一次**，
  ...
  **这一处连着修错了两刀，都是「客户端单边对账」，值得单独记下来当反面教材：**

  - **第一刀**只挡了「擦除手势进行中」。抬笔时在途还有好几份中途快照，闸门一开挨个应用——
    「删掉了又出现，过一会才真的被删除」（比原来的连续闪更像"删除失败"）。
  - **第二刀**改成**按发出批数记账**：发一批擦除帧记一笔，收一次回声销一笔，账没销完就丢弃。
    它靠猜「一批擦除恰好回一次广播」这个**没写进协议的隐含契约**，还得配超时兜底防回声丢失——
    而那个兜底给每笔账各挂一个「到点减一」的定时器，擦除是 8ms 一批，按住擦一秒就是上百笔账、
    上百个定时器，到点后与正常回声**双份销账**，把账提前销光；账一归零，在途的中途快照就被当成
    「最终状态」应用了。用户实测：「删除笔迹当时被删除了，过一会（切了笔模式等）又出现，然后又消失」。

  > **教训（用户当场点破的）：协议有问题就扩协议，别在单边打补丁。** 客户端根本没有分辨
  > 「这份全量快照是不是含我刚发的输入」所需的信息，缺的这条信息只有真源侧给得出——
  > 靠时序、批数、超时去猜，猜法本身就是 bug 的来源。

  **第三刀（当前实现）：扩协议。** `strokes` 帧加首字段 `u32 ackRel` = 生成这份快照时 Mac 已连续
  处理到的**该客户端** REL 序号（`= UDPReorder.relExpected - 1`），按收件人逐连接填在
  `LANServer.rawSend`（每个客户端的可靠流进度不同，不能由 `AppModel` 填一个值发给所有人）。
  客户端判据收敛成一行：

  ```kotlin
  if (ackRel > 0L && sentRel > ackRel) return   // 这份快照比我本地的乐观状态旧 → 丢弃
  ```

  `sentRel` 取 `UdpSender.sentRel`（已发出的最后一个 REL seq）。这条判据：
  - **不需要超时兜底**——`ackRel` 单调递增且由真源给出，最终必然追上；
  - **不依赖任何隐含契约**——Mac 广播几次、什么时候广播，都不影响判据成立；
  - **顺带收掉了乐观笔迹的逐条配对**：快照一旦满足判据，就说明所有已发出的 `ink end` 都已进真源，
    `pendingInk` 整批清空即可（原先那套 FIFO 认领 + 3 秒超时连同 `pendingErase` 一起删掉了）；
  - `ackRel == 0` = 不适用（模式1、浏览器、UDP 会话还没建起来）→ 照单全收，退回旧行为。

  唯一保留的本地判断是「擦除手势进行中一律信本地」：那一批擦除帧可能还没 flush 出去（8ms 一批），
  `sentRel` 还没涨，判据会误判成「含我全部输入」。

**打点**：`PageCanvasView.onDraw` 每 120 帧记一条 `绘制 N帧 均X.Xms 最差Y.Yms 笔迹=N条(可见页 a~b)
几何重建=N条`。「卡不卡」不能靠感觉——均值和最差值是这次唯一能横向对比的凭据。

### 9.10 手写笔侧键「按了没反应，要按好几次」（2026-08-05 用户提，已修）

用户原话：「app 的触控笔按钮很不灵敏，对比网页来说，特别是写字之后切换橡皮擦要按压多次」；
追问后补充的关键条件是——**笔尖一直压在屏幕上不断笔时，按侧键完全没反应**。

这一条排查里我连着提了三个假设，**前两个都被数据推翻**，过程比结论更值得留：

| 假设 | 怎么否掉的 |
|---|---|
| ① 侧键走 `MotionEvent`（`toolType=ERASER` 或 `buttonState` 置位） | 打点实测：按侧键时 `toolType` 恒 2(STYLUS)、`buttonState` 恒 0x0，一动不动 |
| ② 事件没到应用（沉浸式全屏下系统栏抢焦点） | `getevent -lt /dev/input/event13` 证明内核层 DOWN/UP 一直是齐的、网页也收得到；补了 `窗口焦点=` 打点后实测全程 `true` |
| ③ 见下，成立 | 打点覆盖**所有** action 之后，一眼看出来 |

> 打点自身的盲区也栽过一次：第一版 `dispatchKeyEvent` 的日志只在 `ACTION_DOWN` 时打，而下面正好
> 是「DOWN 丢了只剩 UP」——于是用户按了半天，日志里什么都没有，得出「事件没到应用」的错误结论。
> **打点要覆盖全部分支，否则它自己就是下一个盲区。**

真根因是两件独立的事，凑在一起才成了「很不灵敏」：

- **DOWN 会被系统吞掉，只透传 UP。** 真机日志（书写中按三次侧键）：
  ```
  action=1(UP)                 ← 第一次，DOWN 整个没到
  action=0(DOWN) + action=1(UP)  ← 第二、三次齐全
  ```
  典型的系统全局快捷键行为（在 DOWN 上先判要不要自己吃掉）。而 `PadActivity.dispatchKeyEvent`
  此前**只认 `ACTION_DOWN`**，那一次就静默失效。
  改法：**DOWN 与 UP 谁先到就认谁**，用 `KeyEvent.getDownTime()` 去重——一次按下的 DOWN 与它配对的
  UP 共享同一个 `downTime`，拿它判比时间窗精确：既不会同一次触发两遍，也不误吞快速连按的第二下。
- **`cyclePen()` 少了收笔。** `cycleMode()` 一直有 `if (activePen) endPen()`，`cyclePen()` 没有。
  而 `curStrokePen` 是**落笔那一刻**锁进这一笔的，中途换笔不改写已经在画的这条——所以「一直写字
  不断笔时按切笔」`penIndex` 明明变了、画面却毫无变化，看着就是「没反应」。补上收笔即可。

> 排查工具：`adb shell getevent -p` 列输入设备（本机 `event13` = "Xiaomi Smart Pen Keyboard"，
> KEY 位图里的 `0068`/`006d` 就是两个侧键；笔尖是另一个设备 `NVTCapacitiveTouchScreen`）。
> **侧键与笔尖是两个独立设备**，所以 `getevent -lt /dev/input/event13` 能一刀切开
> 「系统/固件没发」和「应用没收到」——这一步省掉了整条往系统层硬钻的弯路。

### 9.11 一个工作区里**只有第一篇文档能有默认图层**（2026-08-05 发现，**两端都中，未修**）

做 §13 多标签页时撞见的：同时开着两篇，第二篇的图层胶囊显示「图层」而不是「图层：Layer 1」。

根因不在安卓这边的新代码，是**跨平台 schema 的语义问题**（Mac 端同款）：

- `ink_layer.id` 是**全局主键**，而默认图层用的是一个**固定** id
  （`00000000-0000-0000-0000-000000000001`，Mac `InkLayer.defaultID` / 安卓 `LibInkLayer.DEFAULT_ID`，
  作用是「payload 里没有 layerId 的老笔迹隐式归到这一层」）；
- `ensureDefaultLayer` 的插入语句两端都是 `ON CONFLICT(id) DO UPDATE SET name/color/sort/visible`
  ——**不更新 `document_id`**。于是第二篇文档执行它时，只是把第一篇那行的名字原样改了一遍，
  自己一行也没有。

模拟器实证（工作区里先后开 doc-a1、doc-a2）：

```
$ sqlite3 library.sqlite "select id,document_id,name from ink_layer;"
00000000-0000-0000-0000-000000000001|doc-a1|Layer 1     ← 只有第一篇有
$ adb logcat | grep 缺默认图层
I/UniReader/Store: doc-a2 缺默认图层，补建一条        ← 补了，但没插进去
```

**影响**：第二篇起的文档图层面板是空的；它的笔迹落库时 `layerId` 仍写这个固定 id，
指向的却是**别人家**的那一行。当前不会丢数据、也不会串图层可见性（可见性过滤按
`inkLayers(documentId)` 取，取不到就是"全可见"），但只要将来按 id 关联去做重命名/删除/改色，
就会跨文档互相影响。

**这是既有 bug，不是多标签页引入的**（从前一次只开一篇，同样只有工作区里第一篇建得起来），
所以没有顺手在这次改动里动它——**默认图层 id 的语义是三端契约，要改得两端一起改**。
两种改法待定：① 默认图层也用随机 UUID，"没有 layerId 的老笔迹"改成兜底到该文档的第一层；
② `ink_layer` 主键改成 `(id, document_id)`（要迁移，且 Mac 端按 id 删除的地方都要带上 document_id）。
...
- **M0 骨架**：加 Pdfium 依赖、包结构重排（`shared/` `pad/` `local/`）、启动页 + 权限引导。
  ...
  # ⚠️ 当时还没有 gradlew，故用 ~/.gradle 缓存里的裸 gradle；2026-08-12 wrapper 已补齐，
  #    现在一律 `cd android && ./gradlew :app:assembleDebug`（见 android/AGENTS.md）
...
| 8 | 把笔迹拖出页边 | **形状不变，整团停在边界上**（2026-08-30 改：整团平移先经 `InkEdit.fitTranslation` 夹位移，不再逐点 clamp 压扁；Mac 与模式1 两条权威路径同改，两端仍一致）；画板模式下往页边深处拖再拖回来，形状同样一丝不变 | 同上 + 回 Mac 比对 |
| 8.1 | **画板模式**下把一团笔迹拖进页边扩展区 | ① 形状不变、不挤成一条（渲染档位本地自愈）；② **选中框贴着笔迹**，不从 PDF 页边起算（包围盒 ±∞ 起算）；③ 松手到归位之间不跳位 | Mac 侧 `touch ~/Library/Logs/UniReader-pad.log`：`框选移动 …` + `页边档位 … → …（pad=true）` 各一行 |
...
| 37 | 用户报的 bug（2026-08-05 **第三版**修＝扩协议加 `ackRel`，三端编译+单测+插桩+跨语言向量过，未真机验）：模式2 下连续快速写好几笔 / **按住橡皮擦一片（要擦够 3 秒以上——第二版正是在这个时长上翻车的）** / 擦完立刻切模式或写一笔（§9.9②） | 连写时上一笔不该闪一下再出现；擦除抬笔后笔迹不该"删掉了又出现、过一会才真的删掉"；**切笔/切模式后已删的笔迹不该复活**；擦完立刻写的那一笔不该消失 | 正常情况**不该**出现 `乐观笔迹 opt:N 等真源超时，撤掉` |
| 38 | 第二版顺带改了笔迹几何的构建方式（§9.9①），**模式1 也走同一份代码** | 模式1 写字/擦除/框选一切照旧，笔迹长相与改动前一致 | 同 36 的绘制打点 |
| 39 | **扩了协议，三端必须一起换**：Mac 与平板都装当天的新版；浏览器采集页也**必须重新打包**（`scripts/build-web.sh`，`capture.html` 里内嵌着 `wire.js`） | 新旧混搭时 `strokes` 帧会整体错位 4 字节——症状是笔迹全乱/解不出来；三端都换新后一切正常 | 平板 `WireCodec` 解不出会静默丢帧；浏览器看 devtools 控制台 |
| 40 | 用户报的 bug（2026-08-05 已修，**用户当日真机验过：切换问题已解决**）：**笔尖压在屏幕上不断笔**时按笔杆侧键（§9.10） | 每一次按下都该生效，不用按好几次（DOWN 被系统吞掉时靠 UP 兜底）；书写中按切笔键会**先收笔**，下一笔立刻是新笔——此前 penIndex 变了但正在画的这条仍是旧笔，看着像没反应 | `侧键 keyCode=… action=… 书写中=… 生效=true/false`：连按两次若第二次 `生效=false`，说明 downTime 去重误判，把日志发我 |
| 41 | **标签页栏的观感与触摸**（§13，模拟器只能证明点得到、点得对，证明不了顺不顺手）：横屏/竖屏各看一遍 | 两条栏（57+44dp）叠起来会不会太吃阅读区；34dp 的 × 会不会误触（想切标签结果关掉了）；标签多了横滑找起来烦不烦；工作区芯片够不够显眼 | — |
| 42 | **切标签页的快慢**（§13 的懒装载/LRU）：工作区放 U 盘/同步盘，来回切三四篇（含被卸载过的那篇） | 保活着的应当是**瞬间**；被卸载过的会重开 Pdfium，慢卷上那一下有多长要有个数（超过 1s 就得考虑把 `MAX_LIVE` 调大或加个「正在打开」的过渡） | `保活上限 3 篇：卸载《…》` / `开 PDF …用了 Xms` / `复原滚动 → 第 N 页 … ok=true` |
| 43 | **多标签页的内存**（§13）：开满 8 个标签页、其中两三篇是几百页的大书，来回切 + 各写几笔 | 不 OOM、不卡；切回来那一屏是**缓存命中**（不该白闪一下再出来） | `缓存上限 → 32MB（前台=false）` / `→ 64MB（前台=true）`；OOM 会有 `Choreographer: Skipped` 与 GC 日志 |
| 44 | **切工作区的耗时**（§13）：两个工作区都放在慢卷上，用芯片来回切 | 「正在打开 X…」压得住（不出现连点开出两个）；切回来标签页组与各篇进度都在原处 | `切换工作区 → …` → `库已关，队列结束` → `开工作区库 … 用了 Xms` → `恢复标签页 N 个` |
| 45 | 用户报的 bug（2026-08-12 已修，编译过未真机验）：**模式1** 按笔杆侧键没反应——§9.10 的 `dispatchKeyEvent` 当时只加在了模式2 的 `PadActivity`，模式1 的 `ReaderActivity` 一直没有这个入口，这次照搬同一份（DOWN/UP 谁先到认谁 + downTime 去重；PageUp 切模式 / PageDown 切笔 / Esc 清框选） | 模式1 每次按下都生效，不用按好几次；书写中按切笔键先收笔、下一笔是新笔（`cyclePen` 的收笔逻辑在共享基类，两模式同一份） | `侧键 keyCode=… action=… 书写中=… 生效=true/false`：连按两次若第二次 `生效=false`，说明 downTime 去重误判，把日志发我 |
| 46 | **双指滚动模式**（2026-08-12 用户需求，§14，编译过未真机验）：模式1 / 模式2 的 ⋯ 里开「双指滚动（防误触）」，然后按平时写字的姿势用一段 | 落笔前虎口/小指蹭到屏幕**不再滑走页面**；两指滚动跟手、同时还能捏合缩放；单指轻点图钉仍能开草稿纸、按住图钉仍能拖；**划一道再抬手不该开出一张草稿纸**（`gestureBlocked`）。关掉开关一切回到原样 | — |
| 47 | **草稿纸上双指整体挪动**（§14.2 顺带修的旧缺陷，两端都改了） | 双指齐挪能拖动纸（此前纹丝不动，只有捏合才动）；双指滚动模式下这条是纸唯一的平移方式，务必确认跟手、不漂 | — |
| 48 | **顶栏新增的 🔒**（§14.3）：横屏/竖屏各看一眼，开着锁双指捏合 | 图标认得出是锁；开着时是 accent 底色；捏合只整体拖动、倍率不变；**杀进程重开后锁与双指滚动都还在**（模式1） | `工具状态已复原：… 锁缩放=true 双指滚动=…`；窄屏被收进 ⋯ 会有 `顶栏宽度不够…收进溢出菜单：锁定缩放` |
| 49 | **模式2 历史设备**（2026-08-12 用户需求，§15，名单规则已在平板上跑过插桩测试，**整条链路未真机验**）：连一次 Mac，退出重进看弹窗顶部；**再把 Mac 的 UniReader 关掉重开**，点同一条 | ① 列表主行是**机器名**（如「xVan 的 MacBook Pro」）而不是 IP——显示 IP 就说明 `/info` 没通，看日志那条；② 点一行直接连上；③ 垃圾桶点了立刻消失、重进不复现；④ **Mac 重开 App 之后点同一条仍能连上**（配对码已持久化，这是这次的重点）；⑤ 面板点「重置配对码」→ 二维码变了、平板被踢，此时点历史条目应当 authFail 且弹窗**带着那台的 host** 重开，重扫后照常连上 | `记入历史设备：<ip>（名字=…）` / `历史设备改名：<ip> → …` / 探测失败会有 `/info 探测失败（<ip>）…` / `移除历史设备：<ip>`；Mac 侧配对码看 `defaults read tech.xvanturing.UniReader pairingToken` |
| 50 | **模式2 页图清晰度**（2026-08-13，§16）：平板不再吃 Mac 写死的 1600px，改按 `pw()` 归档报 `?w=`。Pad 6 横屏 zoom=1 现在拿 2381px（受「窄页最多放大 4 倍」上限，A4 到不了满 2880）。**同一本书跟模式1 并排看** | ① 字比改动前锐；② 与模式1 观感应基本一致（此前模式2 明显糊一档）；③ **翻页/滚动等不等得住**——这是取舍的另一头：模拟器实测高清那趟 537~744ms。真嫌慢就降档，只改 `shared/PageWidths.kt` 的 `STEPS` 末档 | `页图 #N@2880 等 Mac Xms + 下载 Yms + 解码 Zms = Wms，KKKB → 2381x3367 30MB`；Mac 侧 `touch ~/Library/Logs/UniReader-pad.log` 后看 `[PAD]` 行 |
| 51 | **两趟取图的观感**（2026-08-13，§16）：先贴 1080 低清、再换目标档。滚动一段、翻几页 | 占位不再是灰块；**「先糊后清」这一下扎不扎眼**——若觉得跳变明显，把 `PageWidths.PREVIEW` 从 1080 提到 1440（代价是低清那趟也变慢）。高清到货后**不该再变糊**（`hiDone` 判据挡住了低清后到） | 同 50，一页会有**两行**日志（`@1080` 与 `@2880`），低清那行应当明显早到 |
| 52 | **JPEG q85 的画质**（2026-08-13，§16）：默认从 PNG 换成 JPEG，理由是高分辨率下 PNG 编码要 130~260ms 且占 `LANServer` 那条串行队列（字节数**没省**，反多 7~11%） | 扫描件正文/公式/细线上有没有可见压缩痕迹。对照组直接改 URL：`http://<mac>:8770/page.png?i=N&w=2880&f=png` 是无损原样。觉得糊只调 `PageRenderer.defaultJPEGQuality` 一个数 | `栅格 X + JPEG q85 编码 Y，2381×3367px → KKKB` |
| 53 | **模式2 长时间用的内存**（2026-08-13，§16）：位图从 14.5MB/页涨到 30MB/页，缓存改成按字节（堆的 1/3，Pad 6 约 85MB）。连着读半小时、切后台再回来 | 不卡、不闪白；**切后台回来 App 还在**（被系统杀掉就说明 PSS 太高，要么降档要么上区域解码）。模拟器实测 PSS 277MB、native heap 204MB、无 OOM——但那是 8GB 平板之外的口径 | `页图缓存额度 XXMB（堆上限 YYMB）`（连上时打一行）；被杀看 `logcat -b events | grep am_kill` |
| 54 | **新建工作区**（2026-08-30）：启动页「打开工作区」卡上的「新建」→ 选存放位置 → 起名 → 创建。**分别在内部存储、SD 卡、U 盘上各建一个** | ① 建出 `<名字>.unrd/{UniReader/library.sqlite, PDFs/}`；② 建完直接进书库、书库是空的；③ 重名当场报「这里已经有…」而**不覆盖**；④ **建出来的库拿回 Mac 能直接打开**（这条是整件事的验收点） | `UniReader/WS` 的 `新建工作区：…` + `UniReader/Schema` 的 `建库 … schema=v12 name=…` |
| 55 | **添加 PDF**（2026-08-30）：书库右上「＋」→ 浏览到几个 PDF 逐个点。**挑一个 200MB 以上的**，再挑一个放在 U 盘上的 | ① 点下去那行立刻变「添加中…」、完事变「已添加」，**窗不关**能接着加下一本；② 关窗后书库列出这几本，页数正确；③ 大文件期间界面不卡死（hash + 拷贝都在队列线程）；④ 同一个文件再点一次 → 「已在库中」，**不会多出一本书** | `UniReader/Import` 的 `已添加《…》N 页 …B → PDFs/<uuid>.pdf`；`UniReader/StoreQ` 的耗时（超 700ms 带「（慢卷）」） |
| 56 | **建库 + 加书 + 写笔迹一条龙，然后把整个 .unrd 拷回 Mac 打开** | 文档、页数、笔迹、图层全在，Mac 侧不报 schema 错。（宿主机已验：安卓建的 8 张表 / 5 个索引 / 每一列与 Mac 全新 v12 库逐列一致，但**真库跑一遍**才算数） | Mac 侧 `LibraryStore` 不出 `schema_version=…` 警告 |
| 57 | **文案瘦身后的观感**（2026-08-30）：启动页、权限卡、书库空态、目录浏览器 | 长段落换成一行 ⓘ 小提示之后，**该说的还说得清吗**——尤其没授权时那张卡、和第一次进空工作区那一屏 | 无（纯观感） |
| 58 | **扫描页对齐（模式1 只读，2026-09-17，`SCAN-ALIGN-PLAN.md`，编译 + JVM 单测 111/0 过，未真机验；用户同日：「安卓先标记一下待测试」）**：Mac 上对一本扫描书开「对齐扫描页」并写几笔 / 加几条高亮，把工作区给平板打开（直接开或建离线副本） | ① 页面是**转正**的，转的方向与 Mac 相同（不是反向更斜）；② 笔迹 / 高亮 / 图钉位置与 Mac 并排一致；③ 文字清晰度可接受（多了一次重采样，可能略软）；④ 最大缩放档来回滚不卡、不闪退（出图时多一张中间位图）；⑤ 参考窗里看同一本也是转正的；⑥ 离线副本：Mac 上切开关后同步，报告里有「扫描页对齐设置」一条，平板再开跟着变 | `UniReader/Pdf` 的 `…扫描页对齐已开：W=… 戳=…，原始页尺寸与表不符 N 页`（N 应为 0）；`PAGESIZE（扫描页对齐已开…）` |

> **真机（Pad 6，WiFi，竖屏 1800→2160 档）与模拟器的账完全不同，别拿模拟器的数去排优先级**：
> 堆上限 256MB / 缓存额度 85MB / 位图 25MB 一页；**下载+解码只要 97~216ms**（模拟器要 152~770ms，
> 它的解码慢 3~5 倍），而 `等 Mac` 是 **205~716ms**——8 张页图并发时，慢的全在 Mac 那条**串行**
> 服务队列上排队（每张栅格~30ms + JPEG 编码~16ms，串起来就是几百 ms）。
> 也就是说：**真机上的下一个瓶颈在服务端队列，不在平板解码**。要动就是把页图渲染挪出
> `LANServer` 那条 queue（PDFKit 单文档不能并行，但至少别再和 WS/笔迹 RT 抢同一条），
> 这跟区域解码是两件事，别混。
>
> §16 那批（50~53）**没做**第二步「`BitmapRegionDecoder` 只解可见带」：两趟取图上去之后它的性价比明显变差
> （首屏已经不等了，剩下的只是高清早到 ~180ms + 内存 30MB→5MB），而它要动 `shared/PageCanvasView`
> 的绘制热路径、**模式1 也吃同一份代码**。用户 2026-08-13 拍板：先真机用一段，真觉得卡或被后台杀再上。
...
- 标签页的二期（见 §13.7）：拖拽排序、长按菜单（关闭其它/全部关闭）；跨工作区混排一条栏（首版明确不做）。
- **默认图层的 id 语义（§9.11）**：一个工作区里只有第一篇文档建得起默认图层行，两端同病，要一起改。
- Windows 版——schema 已经是跨平台契约，这份文档的 §6 对它同样适用。

## 13. 多标签页 + 快捷切工作区（2026-08-05 用户需求，已落地）

用户原话：「安卓模式1，做下 可以多tab 打开工作区的多个pdf，也可以快捷切换多个工作区」。

### 13.1 两条拍板（问过用户）

- **标签页属于工作区**（不是「浏览器那样跨工作区混排一条栏」）：同一时刻只开**一个**工作区的库，
  切到另一个工作区时整组标签页收起、换成那个工作区自己的一组，**切回来原样恢复**。
  混排要同时挂多份 `library.sqlite` 连接（各带一份 WAL），与 §9.2 的单写者约束顶着来，
  内存与句柄也翻倍——用户选了前者。
- **切工作区的入口 = 标签页栏最左的工作区芯片**（`[考研资料 ⌄]`），点开列「最近打开 + 上次扫描到的」，
  末尾一项「打开其它 .unrd…」跳启动页（目录浏览器与扫描都在那儿，不在阅读界面里重造一遍）。

### 13.2 界面

顶栏（`shared/TopBar`，两模式共用）下面加一条 44dp 的标签页栏 `local/DocTabsBar.kt`：

```
┌──────────────────────────────────────────┐
│ [考研资料 ⌄] │ 线性代数讲义 ×│ 概率论笔记 ×│ + │   ← 44dp，只有模式1 有
├──────────────────────────────────────────┤
│  ◀ ▶   ✎ 笔 尺 文              3/412  ⋯  │   ← 原顶栏 57dp
```

- 工作区芯片与「+」**不随标签横滑**（芯片是切工作区的唯一入口，必须常驻；标签多了中间那段自己滚）。
- 当前标签 = accent 底 + accent 字；重建后自动把当前标签滚进视野。
- **触摸目标这里破例低于 48dp**（芯片高 34、× 命中 34）：顶栏已经 57dp，再叠一条 56dp 就吃掉 113dp
  的阅读区。标签栏是次级导航且每个动作都有替代路径（切标签也能从「+」的列表点，关标签不影响数据），
  **顶栏那条 48dp 下限不动**。观感与误触待真机（§11.1 第 41 条）。
- 「+」弹的是本工作区的文档列表（顺序同书库，与 Mac 一致），已开着的标「已在标签页里」，点它＝切过去。
- 书库列表（`LibraryActivity`）也标出来了：标题变 accent + 行尾「已在标签页里」，顶部计数带「N 个在标签页里」。

### 13.3 架构：`ReaderActivity` 从「一篇文档」变成「一个工作区」

| | 从前 | 现在 |
|---|---|---|
| 库连接 | 每开一篇开一份 | **一个工作区一份**，全部标签页共用同一条 `StoreQueue` |
| 画布 | 一块 `LocalCanvasView` | 一篇一块，同一个 `FrameLayout` 里只有当前那块 `VISIBLE` |
| 打开 | 一跳（库 + Pdfium 同一个后台作业） | **两跳**：库队列上读文档行/路径/笔迹 → 回主线程 → `Bg` 开 Pdfium |
| 进度 | Activity 上一份 | 每个标签页各一份，切走即落库 |

两跳不合并的理由：开 Pdfium 要读**全部**页尺寸表（几百页是秒级），压在库队列上会连带堵住别的标签页的
落笔与进度写入。

**懒装载 + LRU 保活**（内存账，`MAX_LIVE = 3`）：

- 标签页只有被激活过才装载；没装载的在内存里只有 id 和标题。
- 装载过的留着（切回来是瞬间的），超过 3 篇就把最久没看的那篇**卸掉**（关 Pdfium、移除画布）；
  卸载前进度已落库，下次点它就是一次普通的打开，落回原处。
- 退到背景的标签页：`PageCanvasView.trimImages()` 丢掉页图位图（一页 1080×1500 就是 6MB），
  `PdfSource.setForeground(false)` 把 Pdfium 位图缓存从「堆的 1/3」缩到 32MB。
  **不缩就是三份各占堆的 1/3 = 必然 OOM**；缩到 32MB 又刚好留住刚才那一屏，切回来是缓存命中不闪白。
  切回前台必须显式 `refreshImages()`——`GONE→VISIBLE` 尺寸没变不会走 `onSizeChanged`，
  没人调 `ensureImages` 的话那一屏永远补不回来。
- 工具（笔/橡皮/夜间/模式…）是**设备级**的：新开一篇从已经开着的那块画布抄一份（`copyTools`），
  一块都没有才从 `ToolPrefs` 复原。不这么做的话每开一篇都跳回默认笔、夜间模式还会闪一下白。

**队列是全工作区共用的，所以每个作业都得认准自己那一篇**：图层/笔迹/进度的闭包里捏着 `Tab` 与它的
`docId`，作业在队列里排队时用户完全可能已经切到别的标签页了（回调里再用「当前标签页」就写错文档了）。

### 13.4 持久化：`local/TabSet.kt`

`SharedPreferences("tabs")`，**键 = 工作区绝对路径**，值 = `{docs:[document.id…], active:i}`。

- **只存开着哪几篇 + 当前是第几篇，不存进度**：读到第几页/缩放多少是文档自己的属性，真源在
  `document` 的四列上（与 Mac 同一份），存两份必然分叉。
- **不进 `library.sqlite`**：这是本机这台设备的界面状态，塞进库会跟着 `.unrd` 搬到 Mac 上去
  （同 `ToolPrefs` 的理由，§6 的红线）。
- 恢复时**库里没有的 id 直接丢掉**（Mac 上删过文档），否则栏上会挂一个点开就报错的标签页。
- 关掉最后一个标签页 = 退出阅读界面回书库，同时把这条记录删掉（不留空壳）。

### 13.5 入口与返回栈

`ReaderActivity.start()` 带 `REORDER_TO_FRONT | SINGLE_TOP`（manifest 里也声明 `singleTop`）：
已经开着就把**同一个实例**调到前台走 `onNewIntent`（同工作区＝加/切标签页；换了工作区＝整组换掉），
叠第二个实例等于同一个库开两份。**不用 `CLEAR_TOP`**：那会把书库从栈里清掉，返回键就直接回启动页了。

按返回键仍是「退出阅读界面回书库」（标签页组留在 prefs 里，下次开这个工作区照旧恢复）。

### 13.6 已在模拟器上验过的（§11.2 口径：数据/时序/栈状态能证明，观感手感不能）

开 2 个测试工作区（A：3 篇 / B：4 篇）跑完整条链路，全程无崩溃、无 `WindowLeaked`：

- 开/切/关标签页；切走再切回来**页码与滚动位置原样**（5/8 → 1/12 → 5/8）。
- 进度按篇落库（`document.read_page/read_frac` 逐条对过）；标签页组 prefs 逐字对过。
- 切工作区：旧库 `库已关，队列结束` → 新库开出来 → 恢复它自己的那组；切回来又原样恢复
  （含「复原滚动 → 第 5 页 0.441 ok=true」）。
- LRU：开第 4 篇时 `保活上限 3 篇：卸载《…》（最久没看）` + `关闭 b1.pdf`；再点回被卸的那篇能重新装载。
- 前后台缓存额度：`缓存上限 → 64MB（前台=true）` / `→ 32MB（前台=false）`。
- 两个标签页各写一笔，`note` 表里**各自挂在自己的 document_id 上**（共用队列没串档）。
- `打开其它 .unrd…` → 启动页 → 另一个工作区 → 点文档：栈里始终只有**一个** `ReaderActivity`
  记录，走的是 `onNewIntent` → `切换工作区 →`。
- 冷启（`am force-stop` 后重进）标签页组照旧恢复。

### 13.7 没做 / 取舍

- 标签页**上限 8**，满了提示「先关掉一个」——不自动关（悄悄关掉用户开着的东西比拒绝更糟）。
- 不做标签拖拽排序、不做长按菜单（关闭其它/全部关闭）——首版先看用得顺不顺。
- 不做跨工作区混排（见 13.1）。
- 空标签页组（第一次开这个工作区）会**自动弹文档列表**，不停在一块空白上让人猜。

## 14. 防误触：双指滚动模式 + 锁缩放常驻（2026-08-12 用户需求）

用户两条：**① 模式1 的缩放锁定提到顶栏并持久化；② 网站与平板都加「双指滚动」模式减少误触
（双指滚动仍可缩放）**。②「三端都加」是问过用户后定的（逻辑本来就在两模式共用的基类里，
模式1 只多一行菜单声明）。

### 14.1 双指滚动模式在挡什么

已有两道防线挡的是**书写期间**：笔落下后忽略一切手指（`activePen`）、接触面积超
`PadConst.PALM` 的忽略。挡不住的是**落笔之前**——虎口/小指先蹭到屏幕那一下面积不大、笔也还
没 down，于是被当成正经的单指平移，页面直接滑走。

所以这个模式只做一件事：**单指划动不再平移**。滚动与缩放一律双指（双指本来就同时管缩放和整体
拖动，`pinchMove` 的锚点比例始终跟随中点，所以「双指滚动依然可以缩放」是天然成立的，不必另做）。

单指仍然能做的两件事都是**刻意动作**、不会是误触，故不挡：
- 轻点草稿纸图钉 = 打开那张纸；
- 按住图钉拖过死区 = 页内拖动图钉（`pinDragCandidate` 分支在平移判定之前）。

一个坑：单指划一道再抬手，`panStarted` 仍是 false，**会掉进「单击」那条路去开图钉**。所以挡下
的那一下要记一笔（`gestureBlocked` / web 的 `G.gestureBlocked`），抬手时把单击判定一起否掉，
否则误触换个门又进来了。
...
| 端 | 开关放哪 | 实现 |
|---|---|---|
| 模式1 独立版 | ⋯ 菜单「双指滚动（防误触）」，**存 `ToolPrefs`** | 基类 `PageCanvasView.twoFingerScroll` |
| 模式2 输入板 | ⋯ 菜单同名项（不持久化，同该模式其它开关） | 同上，两模式共用这一份 |
| 网页采集页 | 顶栏图标（两个指尖 + 上下箭头），`S.twoFinger` 回显 | `G.twoFinger`，闸门在 `input.ts` 的死区判定处 |

草稿纸那块画布**跟着页内画布走**（宿主每次 `refreshHud`/`refresh` 幂等抄一次
`scratch.canvas.twoFingerScroll`）——用户开的是一个模式，不是两个。

顺带修掉一个原来就有的缺陷：**草稿纸上双指整体挪动拖不动纸**。两处 `pinchMove`（安卓
`ScratchCanvas` / web `scratch.ts`）都只按中点做缩放锚点，两指齐挪时 factor≈1、`zoomAt` 原地
返回，于是纸纹丝不动——页内那份不受影响（它用的是「固定内容比例跟随中点」的算法）。现在两处都
按中点位移补一次平移。不修的话双指滚动模式下草稿纸等于被钉死。

### 14.3 锁缩放常驻

`ReaderActivity`/`PadActivity` 顶栏都加了 🔒 图标（`res/drawable/ic_lock.xml`，几何抄网页那把
锁），原 ⋯ 里的那一项撤掉——窄屏排不下时 `TopBar` 会自己把它收回 ⋯，行为不丢。**两模式一起加**：
顶栏是共用那一份，键的顺序刻意保持一致（`shared/TopBar.kt` 的开篇纪律）。

持久化只做模式1（`ToolPrefs` 新增 `zoomLocked`/`twoFingerScroll`）：模式2 的工具真源在 Mac，
它自己那几个本地开关一向不存。改完即存、不等 `onPause`——设一次用很久的偏好，被系统杀在后台就
白设了（`saveTools()`）。

## 15. 模式2「历史设备」（2026-08-12 用户需求）

用户：**连 Mac 时做历史设备记录，方便选择，设备名就用连接的那台机器的主机名**。

原先连接弹窗只记住**最后一次**成功的 host/token（`prefs("conn")` 两个键），家里/办公室两台 Mac
来回换就得重新扫码或者背 IP。现在每次 authOK 都往名单里记一条，弹窗顶部列出来点一下就连。

### 15.1 机器名从哪来：新增 HTTP `/info`，**不动线格式**

Mac 侧 `LANServer.route` 加一条 `GET /info` →
`{"name":"xVan 的 MacBook Pro","hostName":"xvans-macbook-pro.local"}`
（`Host.current().localizedName` + `ProcessInfo.hostName`，`JSONSerialization` 拼——机器名里
带中文/引号是常态，手拼字符串一定漏转义）。

**为什么不塞进 `authOK`**：线格式动一个字节就是三端同步 + 重出字节向量（`PROTOCOL.md` 开头的
红线），而这只是一句展示用的文本，不值得。不校验 token，与 `/page.png`、`/health` 同级——机器名
在同一局域网里本来就是公开的（Bonjour/SMB 都在广播）。

安卓侧 `KnownMacs.probeName` 在 authOK 之后问一句（OkHttp，3s 超时）。**探不到不是错误**：名字
留空，列表退回显示 IP（`Entry.label`），下次连上再补。连的是**旧版 Mac**（没有这条路由）时也是
这个下场，功能不残废。

### 15.2 名单规则（`pad/KnownMacs.kt`，纯函数那半截有测试）

- 存 `prefs("conn")` 的 `devices` 键（与 host/token/showGraph 同一份——都是「连接设置」）：
  `[{host, token, name, at}]`，最多 8 条，**存的时候就截断**。
- **重连同一台保留已探到的名字**：不然每次重连列表都会先闪回 IP、等 `/info` 回来才变回机器名。
- **换了 IP 就清掉同名的旧地址**（`renamed`）：DHCP 换个地址就多一条僵尸记录（token 还是旧的，
  点了必失败）。代价是同一局域网里两台**电脑名称完全相同**的 Mac 会互相顶掉——真撞上重扫一次码
  就回来，比堆死条目划算。
- **Mac 侧 token 已改成持久**（2026-08-12 用户拍板，`Pairing.persistentToken` 存 `UserDefaults`）：
  原先每次启动现生成，Mac 重开 App 后名单里的 token 就不认了 → 点历史条目必 authFail，名单只剩
  「省了打 IP」这点用。现在同一台 Mac 长期是同一个码，点一下就真能连上。要作废旧码用面板上的
  **「重置配对码」**（`LANServer.resetToken()`，旧码立即失效、连着的平板被踢，需重扫）。
- 即便如此，**authFail 之后弹窗要带着这次尝试的 host/token 重开**（原先是从 prefs 里读最后一次
  **成功**的那组，等于把刚点的那台冲掉了）——重置过配对码、或名单里存着更早的旧码时都会走到这条。
  条目本身不删：IP 通常没变，扫个码就接上。

`KnownMacsTest` 是**插桩**测试而不是 JVM 单测：这几个函数落在 `org.json` 与 `android.util.Log`
上，两者在 JVM 单测里都是空壳，一调就抛 not mocked。

### 15.3 界面

连接弹窗顶部「历史设备」：`ic_monitor` + 机器名 + 灰色一行 IP，右侧垃圾桶移除；点整行 = 回填输入
框并直接连（连不上时弹窗重开，手上还是这组值）。行的样式走 `PadPanels.twoLineRow`——启动页的
「最近打开 / 扫描结果 / 存储卷」原本各写了一份私有实现，这次一并挪进 `shared` 共用。
