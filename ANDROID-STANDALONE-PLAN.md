...
| 34 | 用户报的 bug（2026-08-05 已修，编译过未真机验）：文档笔迹多的情况下滚动/缩放/书写 | 明显不卡了（`PageCanvasView.onDraw` 笔迹层此前无视口裁剪，全文档笔迹每帧重画；已按可见页区间裁剪，同页图/文字铺色口径） | 无专用日志点，肉眼比划顺不顺 + 拿 GPU Profiler/`Choreographer: Skipped` 看有没有明显好转 |
| 35 | 用户报的 bug（2026-08-05 已修，编译+单测过未真机验）：模式1连续快速写好几笔 | 上一笔不该闪一下再出现（`LocalCanvasView.onInkEnd` 改乐观写入 `strokes`，成功不再整篇重读，只有落库失败才读真源纠偏） | `落笔 page=… 点数=…`；写失败会有 `落笔写库失败（这一笔会丢）` 紧跟着笔迹消失 |
