...
| 40 | 用户报的 bug（2026-08-05 已修，编译+单测过未真机验）：**笔尖压在屏幕上不断笔**时按笔杆侧键（§9.10） | 每一次按下都该生效，不用按好几次（DOWN 被系统吞掉时靠 UP 兜底）；书写中按切笔键会**先收笔**，下一笔立刻是新笔——此前 penIndex 变了但正在画的这条仍是旧笔，看着像没反应 | `侧键 keyCode=… action=… 书写中=… 生效=true/false`：连按两次若第二次 `生效=false`，说明 downTime 去重误判，把日志发我 |
