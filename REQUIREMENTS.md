...
- **尺寸跟页缩放**（用户 2026-08-27 拍板）：气泡宽/字号/行高/内边距全是**页宽的比例**，
  比例常数是三端契约（Mac `NoteBubble` / web `render.ts BUB` / 安卓 `NoteBubbleGeom`，
  改一处必须同步另外两处），折行交给各端自己的排版引擎。
  位置规则也是三端一致：图钉右侧优先 → 放不下翻左侧 → 整体钳进页内；超 10 行截断（全文去编辑器看）。
...
- **键盘快捷键**（三端同约定；文本框焦点/带修饰键时一律放行）：
  ...
  - macOS 窗口级：⌘B 切侧栏、⌘I 切检查器（Inspector）
...
- **名字必填**：新建时先弹输入框，**空白/全空格 = 放弃创建**（不落库，同 Mac 端丢弃空点注解的语义）。
  改名走同一个输入框。默认值可以预填「所在章节 · 第 N 页」，但用户仍要按一下确定。
...
- **Mac**：① 阅读区右键菜单「在此添加书签」（锚点 = 右键落点的页 + 页内 frac，`PageStreamView`
  的 `readerContextMenu`）；② **⌘D** = 在当前视口顶处加（全项目未占用，已核）；
  ③ Inspector「目录」页顶部一枚 `+`（同 ⌘D 语义）。书签行右键 = 改名 / 删除。
...
**本轮只定需求。** 实现顺序：**Mac 先行**（数据层 + 目录页合并显示 + 三个入口）→ 网页采集页 →
