...
- **气泡**（`ImageBubbleView`）：复用 `NoteBubble` 的比例常数（宽 = 页宽 × 0.30、圆角/内边距/间隙同款）；里面是
  缩略图（等比缩进气泡宽，高度上限 = 气泡宽，太高的图裁到上限）+ 说明（有才画，最多 3 行）；右上角铅笔进编辑器。
  位置规则同文字气泡（右侧优先 → 放不下翻左侧 → 钳进页内）。
- **展开方式**：`display` 三态与文字笔记完全一致（点击展开/悬浮/始终），点图钉的语义也一致。
- **看大图**：气泡里**双击**缩略图 / Inspector 条目右键「查看」→ `.sheet` 里等比铺满显示（原图分辨率），
