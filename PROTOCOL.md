...
- `strokes` → `{type:"strokes", ackRel, list:[{page, pen:{color,w,t}, pts:[[x,y,pressure],…]},…]}`
  ...
  **`ackRel` = 这份快照里已经体现了该客户端到哪个 REL 序号为止的输入**（§6 的 `seqRel`；
  `0` 表示不适用——没建 UDP 会话的客户端如浏览器，或还没收过任何 REL 包）。
  它按**收件人**逐连接填（`LANServer.rawSend`），因为每个客户端的可靠流进度各不相同。

  > ⚠️ **是「已应用到」，不是「已收到到」**（2026-08-28 修正，两者曾被当成一回事）。Mac 上收帧在
  > 网络队列、**应用**在主线程，中间隔一次 `DispatchQueue.main.async`；快照也是在主线程上建的。
  > 所以取值必须发生在**建快照的那一刻**（`LANServer.broadcast` 从主线程记账的 `appliedRel` 取，
  > 随消息带到发送队列），而不是发送时现取接收进度（`UDPTransport.ackRel`）。取错了就会出现
  > 「快照里还没有那一笔、`ackRel` 却已经盖过它」，客户端据此撤掉自己的乐观笔迹 = 刚写完的字闪一下；
  > 回推越大、发送队列越堵，窗口越宽（用户报「上一个字的笔画依次闪烁」的根因之一）。
  ...
  （用户实测：「删掉了又出现，过一会才真的被删除」）。判据分两件事，**别混成一条**：

  - **擦除**：本端最后一帧 erase 的 `seqRel > ackRel` → 这份快照比本地旧，**整份丢弃**。
    （旧文档写的是「拿本端最后一个 `seqRel` 比」——那会把 ink move 也算进去，连续快写时
    每 8ms 就涨一次，快照于是一份都进不来，见下。）
  - **落墨**：**不丢弃**。落墨是纯追加，旧快照顶多是「少了最后几笔」——照常应用，再把
    `ackRel` 还没追上的那几笔乐观副本原样叠回去即可（各端逐条记下自己 `ink end` 帧的 `seqRel`）。

  > 这条判据同时替掉了客户端侧「按发出批数记账」那类**单边对账**——它靠猜「一批擦除恰好回一次广播」
  > 的隐含契约，还得配超时兜底，安卓端为此翻车两次（补丁史见 `ANDROID-STANDALONE-PLAN.md §9.9`）。
  > `ackRel` 单调递增且由真源侧给出，不需要兜底：它最终必然追上。
  >
